...
def load_data():
    ...
    try:
        ...
        print("Data loaded and initialized successfully via DuckDB.")
    except Exception as e:
        print(f"Startup data loading failure: {e}")

@router.get("/transactions")
def get_transactions(req: TransactionsRequest = Depends()):
    df_filtered = apply_filters('abudhabi_roi', req)
    ...
    if "Sale Application Date" in page_df.columns:
        page_df["Sale Application Date"] = pd.to_datetime(page_df["Sale Application Date"]).dt.strftime("%Y-%m-%d")

    actual_start = pd.to_datetime(df_filtered["Sale Application Date"]).min().strftime("%Y-%m-%d")
    actual_end = pd.to_datetime(df_filtered["Sale Application Date"]).max().strftime("%Y-%m-%d")
...
def get_distribution(req: DistributionRequest = Depends()):
    df_filtered = apply_filters('abudhabi_roi', req)
    if df_filtered.empty:
