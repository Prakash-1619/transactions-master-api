...
def apply_filters_duckdb(table_name: str, filters: dict) -> pd.DataFrame:
    """
    Executes filtering directly against the local DuckDB database.
    This avoids loading Pandas into memory completely.
    """
    ...
    if emirate_filter:
        if emirate_filter.lower() == 'dubai':
            ...
            # We assume emirate exists in unified views, or handle gracefully
    ...
    base_query = f"SELECT * FROM {table_name} {where_sql}"
    ...
    if target_lat is not None and target_lon is not None:
        ...
        if nearest.empty:
            empty_cols = [c for c in RETURN_COLUMNS if c in con.execute(f"DESCRIBE {table_name}").df()['column_name'].tolist()]
            return pd.DataFrame(columns=empty_cols)

        nearest_area = nearest.iloc[0]['area_name_en']
        final_sql = f"{base_query} {'AND' if where_clauses else 'WHERE'} area_name_en = '{nearest_area}'"
    else:
        final_sql = base_query

    result_df = con.execute(final_sql, params).df()
