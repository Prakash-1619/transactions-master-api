...
from latest_rental_data.main import refresh_roi_pipeline

# Construct paths relative to this file's location so it works when run from master_api
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_FILE = 's3://dubai/data/ROI_data/dubai_market_yield_report.csv'
COORDS_FILE = 's3://dubai/data/ROI_data/Area_co_ordinates_rental_areas.csv'

from latest_rental_data.s3_manager import get_s3_fs, get_storage_options

def init_roi_data():
    fs = get_s3_fs()
    # s3fs requires the path without the s3:// scheme for .exists()
    check_path = DATA_FILE.replace('s3://', '')
    if not fs.exists(check_path):
        print("ROI data not found in R2. Initializing...")
        refresh_roi_pipeline()

# --- Utility Functions ---

def load_data(file_path):
    # Backward compatibility for coords_df
    fs = get_s3_fs()
    check_path = file_path.replace('s3://', '')
    if not fs.exists(check_path):
        return pd.DataFrame()
    return pd.read_csv(file_path, storage_options=get_storage_options())
...
def trigger_refresh():
    """
    Manually triggers a re-download and processing of rental and transaction data from R2,
    and recalculates the ROI/Yields.
    """
    try:
        refresh_roi_pipeline()
        return {"status": "Success", "message": "ROI Data successfully refreshed from R2."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/calculate_roi")
async def get_roi(
    ...
    # 3. Locate market row
    query = """
        SELECT * FROM dubai_yield_report 
        WHERE lower(area_name_en) = lower(?)
        AND room_id = ?
        AND year = ?
    """
    match = con.execute(query, [target_area, room_id, year]).df()
    ...
    return {
        ...
        "rooms": row['rooms_en'],
        "year": int(row['year']),
        "roi_yield_percentage": float(round(calculated_yield, 2)),
        "market_reference": {
            "median_rent": float(row['median_annual_rent']),
            "median_price": float(row['median_transaction_amount']),
            "rent_range": {"min": float(row['min_annual_rent']), "max": float(row['max_annual_rent'])}
        }
    }
