...
def init_global_df():
    fs = get_s3_fs()
    dubai_check = DUBAI_CACHE_URL.replace('s3://', '')
    ad_check = ABUDHABI_CACHE_URL.replace('s3://', '')
    
    if fs.exists(dubai_check) and fs.exists(ad_check):
        print("Loading respective emirate market data from R2 cache...")
        storage_opts = get_storage_options()
        required_sales_cols = ['date', 'area', 'property_usage', 'property_type', 'room_type', 
                               'transaction_type', 'reg_type', 'project_name', 'sale_price', 
                               'rate_sqm', 'area_sqm', 'emirate']
        
        try:
            df_dubai = pd.read_parquet(DUBAI_CACHE_URL, storage_options=storage_opts, columns=required_sales_cols)
            df_ad = pd.read_parquet(ABUDHABI_CACHE_URL, storage_options=storage_opts, columns=required_sales_cols)
            return pd.concat([df_dubai, df_ad], ignore_index=True)
        except Exception as e:
            # Fallback to reading all if columns don't match, or handle memory error
            print(f"Failed to read specific columns, falling back... ({e})")
            df_dubai = pd.read_parquet(DUBAI_CACHE_URL, storage_options=storage_opts)
            df_ad = pd.read_parquet(ABUDHABI_CACHE_URL, storage_options=storage_opts)
            return pd.concat([df_dubai, df_ad], ignore_index=True)
    else:
        print("R2 cache not found, processing unified data from raw files...")
...
# Initialize data
global_df = init_global_df()

global_rental_df = pd.DataFrame()
RENTAL_DATA_PATH = 'c:/Users/pooja/Transactions_api/rental_df.parquet'
if os.path.exists(RENTAL_DATA_PATH):
    print("Loading global rental data from parquet...")
    try:
        # Load only necessary columns to prevent ArrowMemoryError
        required_cols = ['contract_start_date', 'annual_amount', 'area_name_en', 'ejari_property_sub_type_en', 'ejari_property_type_en']
        global_rental_df = pd.read_parquet(RENTAL_DATA_PATH, columns=required_cols)
        if 'contract_start_date' in global_rental_df.columns:
            global_rental_df['contract_start_date'] = pd.to_datetime(global_rental_df['contract_start_date'], format="%d-%m-%Y", errors='coerce')
        print("Rental data loaded successfully!")
    except Exception as e:
        print(f"Failed to load rental data: {e}")
...
def refresh_data():
    global global_df
    try:
        global_df = reprocess_data()
        return {"status": "success", "message": "Unified market data refreshed from S3 and cached."}
    except Exception as e:
...
def get_market_insights(
    ...
    if lat is not None and lon is not None and not coords_df.empty:
        ...
        if not temp_coords.empty:
            ...
            if nearest['dist'] <= DEFAULT_RADIUS_KM:  # Use config radius threshold
                ...
                df = df.iloc[0:0] # Return empty df to match standard format instead of error dict

    # Filter by Emirate (either manually provided or inferred)
    if emirate:
        df = df[df['emirate'].str.lower() == emirate.strip().lower()]
    elif not emirate and (lat is None or lon is None):
        # If no emirate is provided and no coordinates were given, return empty df
        df = df.iloc[0:0]
    
    # Apply the area filter if coordinates successfully inferred it, else use manual area filter
    if inferred_area:
        df = df[df['area'].str.lower() == inferred_area]
    elif area:
    ...
    if reg_type:
        df = df[df['reg_type'].str.lower() == reg_type.strip().lower()]
    if transaction_type:
        df = df[df['transaction_type'].str.lower() == transaction_type.strip().lower()]
    if room_type:
        df = df[df['room_type'].str.lower() == room_type.strip().lower()]
    if property_type:
        df = df[df['property_type'].str.lower() == property_type.strip().lower()]
...
def get_market_trends(
    ...
    if lat is not None and lon is not None and not coords_df.empty:
        ...
        if not temp_coords.empty:
            ...
            nearest = temp_coords.sort_values('dist').iloc[0]
            from config import DEFAULT_RADIUS_KM
            if nearest['dist'] <= DEFAULT_RADIUS_KM:
                inferred_area = str(nearest['area_name']).lower()
                emirate = nearest['emirate_source']
            else:
                df = df.iloc[0:0]
    ...
    periods_map = {"mom": 1, "qoq": 3, "yoy": 12}
    shift_period = periods_map.get(comparison_type.lower(), 12)
...
def get_rental_trends(
    ...
    if inferred_area:
        df = df[df['area_name_en'].astype(str).str.lower() == inferred_area]
    elif area:
        df = df[df['area_name_en'].astype(str).str.lower() == area.strip().lower()]

    if start_date:
        df = df[df['contract_start_date'] >= pd.to_datetime(start_date)]
    if end_date:
        df = df[df['contract_start_date'] <= pd.to_datetime(end_date)]
    
    if room_type and 'ejari_property_sub_type_en' in df.columns:
        df = df[df['ejari_property_sub_type_en'].astype(str).str.lower() == room_type.strip().lower()]
    if property_type and 'ejari_property_type_en' in df.columns:
        df = df[df['ejari_property_type_en'].astype(str).str.lower() == property_type.strip().lower()]
        
    if df.empty:
        return {"message": "No data found for the given criteria."}
        
    df['Month_Year'] = df['contract_start_date'].dt.to_period('M')
