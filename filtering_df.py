...
def load_and_prep_data(trans_path, coords_path):
    # Load transactions directly from parquet via S3 (or locally)
    print(f"Loading Dubai data from: {trans_path}")
    if str(trans_path).startswith("s3://"):
        df = pd.read_parquet(trans_path, storage_options=get_storage_options())
    else:
    ...
    # Load coordinates and merge if available
    if os.path.exists(coords_path):
        coords_df = pd.read_csv(coords_path)
        coords_df = coords_df.rename(columns={'lat': 'latitude', 'long': 'longitude'})
        combined_df = df.merge(coords_df[['area_name', 'latitude', 'longitude']],
                               left_on='area_name_en', right_on='area_name', how='left')
        return combined_df.drop(columns=['area_name'])
    else:
        print(f"Warning: {coords_path} not found. Skipping coordinate mapping.")
        df['latitude'] = np.nan
        df['longitude'] = np.nan
        return df
...
def apply_filters(df, filters: dict):
    ...
    if target_lat is not None and target_lon is not None:
        # Extract dynamic radius from input, default to DEFAULT_RADIUS_KM if not provided
        radius_val = filters.get('radius')
        radius_km = float(radius_val) if radius_val is not None else DEFAULT_RADIUS_KM
...
def get_processed_df(trans_path, coords_path):
    fs = get_s3_fs()
    check_path = CACHE_FILE.replace('s3://', '')
    if fs.exists(check_path):
        ...
        try:
            ...
            s3_fs = get_s3_fs()
            dataset = pq.ParquetDataset(CACHE_FILE.replace('s3://', ''), filesystem=s3_fs)
            available_parquet_cols = dataset.schema.names
            cols_to_load = [c for c in set(RETURN_COLUMNS + ['latitude', 'longitude', 'actual_worth', 'meter_sale_price', 'procedure_area']) if c in available_parquet_cols]
            return pd.read_parquet(CACHE_FILE, storage_options=get_storage_options(), columns=cols_to_load)
        except Exception as e:
            print(f"Failed to load optimized subset, trying full load: {e}")
            return pd.read_parquet(CACHE_FILE, storage_options=get_storage_options())
    
    # Generate cache if it doesn't exist
    return reprocess_and_save(trans_path, coords_path)

def reprocess_and_save(trans_path, coords_path):
    print("Processing raw data and saving to cache...")
    raw_df = load_and_prep_data(trans_path, coords_path)
    df_clean1 = clean_area_outliers(raw_df)
    df_final = remove_granular_outliers(df_clean1)
    
    df_final.to_parquet(CACHE_FILE, storage_options=get_storage_options(), index=False)
